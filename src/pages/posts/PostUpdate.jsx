...
const PostUpdate = () => {
    ...
    const handleSubmit = async (e)=>{
        ...
        try {
            setLoading(true);
            const {data} = await axiosConnection.put(`/api/posts/${id}/${formData.userId}`, formData, {
                headers:{
    ...
    const fetchPost = async ()=>{
        ...
        try {
          setLoading(true)
          const {data} = await axiosConnection.get(`api/posts?postId=${id}`);
  ...
  return (
    <main className='p-3 max-w-3xl mx-auto min-h-screen'>
        <h1 className='text-center font-bold text-2xl my-7'>Update Post</h1>
        ...
        <form className='space-y-5' onSubmit={handleSubmit}>
            ...
            <div className='flex gap-2 flex-col md:flex-row'>
                ...
                <Select id="category" required onChange={handleChange} value={formData.category}>
            ...
            <div className='flex justify-start items-center'>
                ...
                {formData.image && (
                    <img  className='w-28 h-28 object-fill rounded-md' src={formData.image} alt="img file" />
                )}
            </div>
